...
root.iconbitmap("img/youtube1.ico")
root.resizable(False,False)
canvas=Canvas(root,width=600,height=100)
canvas.grid(row=0,column=0)

Banner=ImageTk.PhotoImage(Image.open("img/banner.png"))
...
canvas=Canvas(tab1,width=300,height=200,bg="#f0f0f0")
...
def get():
	...
	try:
		...
		if click=="GET VIDEO":
			...
			resolution= [stream.resolution for stream in yt.streams.filter(file_extension="mp4",progressive=True)]
...
def download():
	...
	if VIDEO==TRUE:
		...
		try:
			...
			size=file.filesize
			a=messagebox.askyesno("Do You Want To Download",f"File Size: {round(size* 0.000001, 2)} mb")

			if a==True:
				file=download_dir+yt.title
		...
		try:
			...
			if get==True:
				...
				try:
					...
					messagebox.showinfo("Youtube Downloader",f"A File Already Exists with SAME NAME\nDelete or Rename The Existing File in\n\nPath: {download_dir}\n\nDownloaded File Moved To Trash")
					send2trash(converted)
...
button1=Button(tab1,text="GET VIDEO",font=("Arial Bold",10),bg="#ccccff",command=lambda b="GET VIDEO":thread(b))
button1.grid(row=1,column=0,ipadx=50,ipady=10)

button2=Button(tab1,text="GET AUDIO",font=("Arial Bold",10),bg="#ccccff",command=lambda b="GET AUDIO":thread(b))
button2.grid(row=1,column=1,ipadx=50,ipady=10,pady=5)

button3=Button(tab1,text="Download",font=("Arial Bold",10),bg="#ccccff",command=thread1)
button3.grid(row=8,column=0,ipadx=50,ipady=15,pady=5)

download_path=StringVar()
download_dir = str(Path.home() / "Downloads")
download_path.set(download_dir)
Button(root,text="Browse Path",font="Helvetica 15 bold",command=Browse).grid(pady=2)
Entry(root,textvariable=download_path,width=40,font='Helvetica 15 bold').grid(pady=5, row=9,column=0)
